...
import com.dongyang.ui.TTableSortNode;
import com.dongyang.ui.TTextFormat;

public class TTableSortBase extends TScrollPane
{
    /**
     * 内部Table对象
     */
    private JTableSortBase table;
    /**
    ...
    public TTableSortBase() {
        ...
        TableModel dm = new TTableSortModel();        
        //
        TTableSortColumnModel cm = new TTableSortColumnModel(this);
        TTableSortSelectionModel sm = new TTableSortSelectionModel(this);       
        JTableSortBase table=new JTableSortBase(dm,cm,sm);  
        //加入排序
        ...
        //设置显示适配器
        setCellRenderer(new TTableSortCellRenderer(this));
        dataStore = new TDS();
    }
     ...
     */
    public void onChangeValuePrivate(TTableSortNode node)
    {
        //是否自动更新属性对象
     ...
     */
    public void setTable(JTableSortBase table)
    {
        if(this.table == table)
            return;
        if(this.table != null)
     ...
     * @return JTableBase
     */
    public JTableSortBase getTable()
    {
        return table;
    }
    /**
     ...
     * @return TTableModel
     */
    public TTableSortModel getModel()
    {
        if(getTable() == null)
            return null;
        return (TTableSortModel)getTable().getModel();
    }
    /**
     * 得到列数据
     * @param value String[]
    ...
    {
        TTableSortModel model = getModel();
        if(model == null)
            return;
     ...
     */
    public TTableSortColumnModel getColumnModel() {
        if(getTable() == null)
            return null;
        return (TTableSortColumnModel)getTable().getColumnModel();
    }
    /**
     * 得到列对象
     * @param index int
    ...
    public void setHeader(String header)
    {
        ...
        for(int i = 0;i < columns.length;i++)
        {
            TTableSortCellEditor editor = new TTableSortCellEditor(this,(String)columnTypeData.get(i));
    ...
    public TParm getShowParmValue()
    {
        ...
        TTableSortCellRenderer cell = getCellRenderer();
    ...
    public void removeRowAll(){
    	//假如支持排序;      	
	        ...
	        ((TTableModel) table.getModel()).getDataVector().clear();
	        ((TTableModel) table.getModel()).fireTableDataChanged();
	        //清parm
	        if(getParmValue() != null){
	        	for(int i = getRowCount() - 1;i >= 0;i--){
	                getParmValue().removeRow(i);
	        	}
	        }
	        //清datastore
	        if(isAutoModifyDataStore()){
	        	for(int i = getRowCount() - 1;i >= 0;i--){
	        		 getDataStore().deleteRow(i);
	        	}            
    ...
    public TTableSortCellEditor getCellEditor(int column)
    {
        TableColumn columnObject = getColumn(column);
        if(columnObject == null)
            return null;
        return (TTableSortCellEditor)columnObject.getCellEditor();
    }
    /**
     * 组件个数
     * @return int
    ...
    {
        TTableSortCellRenderer cellRenderer = getCellRenderer();
    ...
    public void setDataStore(TDataStore dataStore)
    {
        ...
        if(dataStore instanceof TDS)
        {
            ...
            if(ds.getTableSort()!= this)
                ds.setTableSort(this);
        }
    }
    /**
     * 得到数据存储
