 ...
 * @version 1.0
 */
public class EHasChoose extends EFixed
{
    /**
     * 数据
     */
     ...
     * @param panel EPanel
     */
    public EHasChoose(EPanel panel)
    {
        super(panel);
    }
    /**
    ...
    {
        return getPanel().newHasChoose(index);
    }
    /**
     * 得到坐标位置
     * @return String
     ...
     * 值选择对话框
     */
    public void openValuePopupMenu()
    {
        ...
        TWindow propertyWindow = (TWindow)getPM().getUI().openWindow("%ROOT%\\config\\database\\HasChoosePopMenu.x", getHeadFixed() ,true);
    ...
    public void openPropertyDialog()
    {
        ...
        String value = (String)getPM().getUI().openDialog("%ROOT%\\config\\database\\HasChooseEdit.x",fixed);
    ...
    {
        EHasChoose hasChoose = new EHasChoose(panel);
        hasChoose.setStart(panel.getDString().size());
        hasChoose.setEnd(getStart());
        hasChoose.setString(getString());
        hasChoose.setKeyIndex(getKeyIndex());
        hasChoose.setDeleteFlg(isDeleteFlg());
        hasChoose.setStyleOther(getStyle());
        hasChoose.setName(getName());
        hasChoose.setGroupName(getGroupName());
        hasChoose.setMicroName(getMicroName());
        hasChoose.setDataElements(isIsDataElements());
        hasChoose.setAllowNull(isAllowNull());

        for(int i = 0;i < sizeData();i++)
        {
            Vector d = (Vector)getData().get(i);
            hasChoose.addData((String)d.get(0),(String)d.get(1));
        }
        return hasChoose;
    }
}
